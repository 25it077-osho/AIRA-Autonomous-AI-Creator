...
export const API_BASE_URL =
  ...
  'https://aira-autonomous-ai-creator.onrender.com'

const AGENT_ID_STORAGE_KEY = 'aira_agent_id'
const PERSONA_STORAGE_KEY = 'aira_persona'
...
export async function initializeAgent(persona) {
  ...
  if (!data?.agentId) {
    throw new ApiError(
      'Backend did not return an agentId.',
      { kind: 'parse' }
    )
  }
...
export async function getFeed(agentId) {
  if (!agentId) {
    throw new ApiError(
      'No agentId available — initialize the agent first.',
      { kind: 'unknown' }
    )
  }
